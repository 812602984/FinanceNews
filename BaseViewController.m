...
-(void)loadDataPage:(NSInteger)page count:(NSInteger)count{
    //获取url
    NSString *url = [NSString stringWithFormat:kUrl,self.pid];
    ...
    [_manager GET:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (responseObject) {
            ...
            if (![weakSelf.pid isEqualToString:@"101002114"]){
                if (_urlArr.count>2) {
                    [weakSelf addAdView:_imageNames titleArr:weakSelf.textArr];
